...
namespace ManagemateAPI.Management.M_Item.Manager
{
    public class Item_Manager
    {
        ...
        //Add
        public async Task<string> AddItem(Add_Item_Data item)
        {
        ...
        public async Task<string> Edit_Item(Edit_Item_Data item)
        {
            ...
            else
            {
                if (await Session_Checker.ActiveSession(item.session))
                {
                    ...
                    {
                        var cat_num_exists = _context.Item.Where(i => i.catalog_number == item.catalog_number).FirstOrDefault();
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                edited_record.item_trading_type_id_FK = item_trading_type;
                ...
                if (await Session_Checker.ActiveSession(obj.session))
                {
                    ...
                    else
                    {
                        List<Encrypted_Object> encrypted_fields = [
                            ...
                            new Encrypted_Object { id = 3, encryptedValue = selected_record.item_type_id_FK.item_type }
                        ];
        ...
        //Get All Items
        public async Task<List<Item_Model_List>> Get_All_Items(Get_All_Items_Data input)
        {
            if (input == null)
            {
                throw new Exception("14");
